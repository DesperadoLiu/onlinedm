...
const MSG_TURNSTILE_MISSING = 'Security check is not configured. Please contact support.';
const MSG_TURNSTILE_FAIL = 'Security check failed. Please try again.';
const MSG_TURNSTILE_TIMEOUT = 'Security check timed out. Please try again.';
...
function createVisitorId() {
    ...
    if (window.crypto && typeof window.crypto.getRandomValues === 'function') {
        ...
        return `v_${Array.from(bytes, (byte) => byte.toString(16).padStart(2, '0')).join('')}`;
    }
...
function getOrCreateVisitorId() {
    try {
        ...
        if (!window.__jjkVisitorId) window.__jjkVisitorId = createVisitorId();
        return window.__jjkVisitorId;
    }
}

async function waitForTurnstileApi() {
    const start = Date.now();
    while (Date.now() - start < TURNSTILE_TIMEOUT_MS) {
        if (window.turnstile && typeof window.turnstile.render === 'function') return;
        await new Promise((resolve) => window.setTimeout(resolve, 100));
    }
    throw new Error(MSG_TURNSTILE_TIMEOUT);
}

function getTurnstileSiteKey() {
    const mount = document.getElementById('turnstileMount');
    const siteKey = mount?.dataset?.sitekey ? mount.dataset.sitekey.trim() : '';
    if (!siteKey) throw new Error(MSG_TURNSTILE_MISSING);
    return siteKey;
}
...
async function getTurnstileToken() {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeoutId = window.setTimeout(() => {
            ...
            holder.remove();
            reject(new Error(MSG_TURNSTILE_TIMEOUT));
...
function getFriendlyErrorMessage(error) {
    if (!error) return MSG_SERVICE_DOWN;
    if (error.name === 'AbortError') return 'Request timed out. Please try again.';

    const rawMessage = typeof error.message === 'string' ? error.message.trim() : '';
    if (!rawMessage) return MSG_SERVICE_DOWN;

    if (rawMessage === 'Failed to fetch') {
        if (navigator.onLine === false) return 'Network disconnected. Please check your internet connection.';
        return 'Connection blocked. Please check GAS Web App access (Anyone), deployment version, and browser console CORS errors.';
    }

    if (rawMessage.startsWith('Backend HTTP 403')) {
        return 'Access denied by backend (HTTP 403). Please check GAS Web App access permission.';
    }
    if (rawMessage.startsWith('Backend HTTP 404')) {
        return 'Backend endpoint not found (HTTP 404). Please check GAS_WEB_APP_URL.';
    }
    if (rawMessage.startsWith('Backend HTTP 429')) {
        return 'Too many requests. Please wait a moment and try again.';
    }
    if (rawMessage.startsWith('Backend HTTP 5')) {
